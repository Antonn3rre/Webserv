...
bool generateAutoindex(const std::string &page, std::string &body) {
...
ResponseMessage RequestHandler::_createResponse(const Config &config, RequestMessage &request,
	...
	if (handled.first == 1) {
		body = CgiHandler::executeCgi(request, handled.second, config, 3);
		handled.first = 200; // code a revoir
	} else if (handled.first == 2) {
		handled.first = 200;
		if (!generateAutoindex(handled.second, body)) {
